...
public interface CustomerInquaryDAO extends JpaRepository<CustomerForm, String>{
	...
	@Query("select ut.userid from User_Details_Table ut where ut.pos_table=:pos AND ut.role IN('SALESMAN','SALES EXECUTIVE')")
	public List<Object> findsalesman(@Param("pos") POS_Table pos);

	@Query("select ut.userid from User_Details_Table ut where ut.pos_table=:pos AND ut.role IN('SALESMAN','SALES EXECUTIVE')")
	public List<String> findsalesmanbypos(@Param("pos") POS_Table pos);
}
