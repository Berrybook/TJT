...
public interface KisokInvoiceDAO extends JpaRepository <KiskoInvoice, Long>  {
	...
	@Query("select ig.customerID,ig.user_Details_Table,ig.saledate,ig.totalprice from KiskoInvoice ig where ig.user_Details_Table IN (SELECT userid from User_Details_Table st WHERE st.pos_table=:postable)")
